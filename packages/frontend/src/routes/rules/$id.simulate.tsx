       ...
       <Group justify="space-between" mb="lg">
        <div>
          <Group gap="xs">
             ...
             <Badge variant="light" size="lg">{id}</Badge>
          </Group>
          <Text c="dimmed">Test rule logic with simulated inputs.</Text>
        </div>
        <Group>
            <Button variant="default" component={Link} to={`/rules/${id}`}>Back to Editor</Button>
            <Button 
      ...
      <Grid>
        ...
        <Grid.Col span={{ base: 12, md: 5 }}>
            <Card withBorder h="100%">
                <Title order={4} mb="md">Input Context</Title>
                <Stack>
                    <Select 
                        label="Load Scenario"
            ...
            <Stack>
                ...
                <Card withBorder>
                    ...
                    {result ? (
                        ...
                        <Text c="dimmed" size="sm" fs="italic">Run simulation to see results...</Text>
                    )}
                </Card>

                {/* Side Effects Timeline */}
                <Card withBorder title="Side Effects">
                    <Title order={4} mb="md">Effects Timeline</Title>
                    
                    {!result ? (
                         <Text c="dimmed" size="sm" fs="italic">No effects generated yet.</Text>
                        ...
                        <Timeline active={result.actions.length - 1} bulletSize={24} lineWidth={2}>
                            {result.actions.map((action: any, index: number) => (
                                <Timeline.Item 
                                    key={index}
                                    bullet={
                                        action.type === 'UpdateState' ? <IconDatabase size={12} /> : <IconBolt size={12} />
                                    }
                                    color={action.type === 'UpdateState' ? 'teal' : 'blue'}
                                    title={action.type}
                                >
                                    <Text c="dimmed" size="xs" tt="uppercase" fw={700}>{action.target}</Text>
                                    <Text size="sm" mt={4}>
                                    ...
                                    <Paper withBorder p="xs" mt="xs" bg="var(--mantine-color-gray-0)">
                                        <Code block fz="xs" bg="transparent">{JSON.stringify(action.payload, null, 2)}</Code>
                                    </Paper>
                                    <Text size="xs" mt={4} c="dimmed">{action.timestamp}</Text>
                                </Timeline.Item>
                            ))}
                        </Timeline>
                    )}
