...
export class OpaLoadError extends Data.TaggedError("OpaLoadError")<{
  message: string
  cause?: unknown
}> {}
...
export class Security extends Effect.Service<Security>()("app/Security", {
  effect: Effect.gen(function* () {
    ...
    const policy = yield* Effect.tryPromise({
      ...
      catch: (err) => new OpaLoadError({
        message: `Failed to load OPA policy: ${err instanceof Error ? err.message : String(err)}`,
        cause: err
      })
    });
