...
export const TestProject = Entity.make("TestProject", [
  Rpc.make("SubmitProject", {
    success: Project,
    payload: { id: Schema.Number }
  }),
  Rpc.make("ValidateProject", {
    success: Project,
    payload: { id: Schema.Number }
  }),
...
export const TestEntityNoState = TestProject.toLayer(
  Effect.gen(function*() {
    ...
    return {
      SubmitProject: (envelope) =>
        Effect.gen(function*()  {
          ...
          // }
          console.log(`Submitting project at state time: ${state}`)
          yield* Effect.log(`Submitting project at state time: ${state}`)
          return new Project({ id: envelope.payload.id, name: `Project ${envelope.payload.id}` })
        }),
      ValidateProject: (envelope) =>
        Effect.gen(function*() {
          // state.envelopes.unsafeOffer(envelope)
          // if (state.defectTrigger.current) {
          //   MutableRef.set(state.defectTrigger, false)
          //   throw new Error("User not found")
          // }
          console.log(`Validating project at state time: ${state}`)
          yield* Effect.log(`Validating project at state time: ${state}`)
          return new Project({ id: envelope.payload.id, name: `Project ${envelope.payload.id}` })
        }),
